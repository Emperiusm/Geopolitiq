...
> **Status:** Complete — 10/10 tasks. Branch: `feature/phase-1-engine-foundation` (merged to main).
> **Spec:** `docs/v2/phase-1-engine-foundation.md` | **Plan:** `docs/v2/phase-1-implementation-plan.md`
...
> **Status:** Complete — 23/23 tasks. Branch: `feature/phase-2-ingestion-framework` (merged to main).
> **Spec:** `docs/v2/phase-2-ingestion-framework-design.md` | **Plan:** `docs/v2/phase-2-implementation-plan.md`

- Build Temporal workflow pipeline
- Migrate RSS news pipeline as first source
- Add first behavioral sources (USPTO, SEC EDGAR, USASpending, Jobs, Lobbying, Federal Register, Semantic Scholar, FCC)
- Entity resolution service against Typesense
- Signal writing to PostgreSQL + ClickHouse sync
- Self-learning parser promotion, DLQ triage, OpenTelemetry observability

### Phase 3a: Gap Analysis & Detectors

> **Status:** Planned — 19 tasks.
> **Spec:** `docs/v2/phase-3a-gap-analysis-detectors-design.md` | **Plan:** `docs/v2/phase-3a-implementation-plan.md`

Computation layer — scoring engine, detectors, workflow orchestration:
- Gap scoring algorithm: signal weights (decay, tier, financial), alignment, category classification (confirmed/stealth/vaporware/pivot/insufficient-data/unclassified), Reality Score sigmoid normalization, composite confidence, peer group percentiles
- Signal-driven recompute via Redis sorted set coordinator (polls every 30s, natural dedup) + 6-hour scheduled sweep as safety net
- Batched chunk workflows (100 pairs/chunk, max 10 concurrent) with advisory locks, ClickHouse fallback for signal reads
- 5 core detectors via pluggable registry: Stealth Project, Vaporware, Acquisition Target (Neo4j, entity-type-aware rubrics), Contradiction, Trend Reversal
- Separate DetectorBatchWorkflow with `shouldRun` pre-filtering, circuit breakers, cooldown, evidence fingerprinting
- Graph snapshots: tiered materialization (hot/warm/cold), 2-hop Neo4j subgraphs, Cosmograph-ready node/edge format
- API routes: `/gaps` (leaderboard, movers, domain), `/domains` (taxonomy CRUD), entity score/history endpoints, admin recompute
- ClickHouse materialized views for leaderboard, daily pre-aggregated score history
- Algorithm versioning with shadow mode migration strategy

### Phase 3b: Alerts & Watchlists

> **Status:** Not yet specced. Depends on Phase 3a.

Notification/user-facing layer — alert management, watchlist matching, delivery:
- AlertService: alert CRUD, status lifecycle (new → acknowledged → investigating → dismissed/confirmed), team-scoped queries
- WatchlistService: watchlist CRUD, entity add/remove via `watchlist_entities` junction table, notification config
- Watchlist fan-out: when detector generates alert, match against `watchlist_entities` to find teams to notify
- Alert delivery: SSE (immediate via `sse:alerts:{teamId}`), webhook (via existing `WebhookPublisher` + Temporal delivery workflow), email (queued)
- API routes: `/alerts` (list, patch status, SSE feed), `/watchlists` (CRUD, entity management), entity `/:id/alerts`
- Alert expiration: 90-day TTL for dismissed/confirmed, periodic cleanup workflow
- Endpoint-specific rate limiting for alert/watchlist queries

### Phase 4: Signal Dashboard
- Build React + Tailwind frontend
- Leaderboard, Entity Detail, Alert Feed, Comparison view
- Spatial View (globe as React component via Deck.GL)
- SSE streaming for real-time updates
- Cosmograph graph visualization (consumes graph snapshots from Phase 3a)
- Signal-driven gap recompute trigger integration (real-time score freshness for dashboard)

### Phase 5: Advanced Capabilities
- FOIA intelligence system (automated filing, corpus scraping, response intelligence, cascading requests)
- Advanced detectors (ghost listings, patent examiner behavior, domain registration timing, academic co-authorship, supplier triangulation, conference withdrawal, environmental filing, "dog that didn't bark")
- Cross-entity pattern detection (competitor convergence, supply chain cascade, sector rotation)
- Additional domain sources
- Export, GraphQL (Yoga), billing
