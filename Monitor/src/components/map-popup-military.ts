...
import { getOperatorCountryCode, getFlagEmoji, formatCoord, OPERATOR_COUNTRY_MAP } from './map-popup-helpers';
...
function renderClusterVesselItem(v: MilitaryVessel): string {
  const code = getOperatorCountryCode(v);
...
export function renderMilitaryVesselPopup(vessel: MilitaryVessel): string {
  ...
  const countryCode = getOperatorCountryCode(vessel);
            ...
            <div class="vessel-history-list">
              ${vessel.track.slice(-5).reverse().map((tp, i) => `
                <div class="vessel-history-item">
                  <span class="history-point">${formatCoord(tp[0], tp[1])}</span>
...
export function renderMilitaryVesselClusterPopup(cluster: MilitaryVesselCluster): string {
  ...
  const dominantCode = dominantOp ? (OPERATOR_COUNTRY_MAP[dominantOp] || '') : '';
  const dominantFlag = dominantCode ? getFlagEmoji(dominantCode) : '';

  const visibleVessels = cluster.vessels
    .slice(0, 5)
    .map(v => renderClusterVesselItem(v))
    .join('');
